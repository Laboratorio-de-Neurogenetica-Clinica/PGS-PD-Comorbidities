...
PRS_PRS <- function(){
    ...
    data <- merge(merge(merge(merge(merge(load_PRS_data("EPI"),load_PRS_data("T2D"),by="FID"),
                                    load_PRS_data("MH"),by="FID"),load_PRS_data("MDD"),by="FID"),
                        load_PRS_data("Height"),by="FID"),
                  fread("EPI_PRS_pheno_covar.tsv")[,c(1,14,21)], by="FID" )
    data <- data[data$AGE >= 50]
    
    data_PD <- data[data$PD == 1 ]
    colnames(data_PD) <- c("FID","PGS EPI (zSCORE)","PGS T2D (zSCORE)","PGS MH (zSCORE)","PGS MDD (zSCORE)","PGS Height (zSCORE)","AGE","PD")
