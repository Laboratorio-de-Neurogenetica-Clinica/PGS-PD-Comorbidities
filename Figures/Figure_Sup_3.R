...
library(rstatix)
library(magrittr)
############################### Figure sup 3 , General Population, VIOLINPLOTS, DENSITY PLOT y DECILE PLOT
...
Violin_plots <- function (PRS,Disease,subset,age=0,test="t.test") {
    ...
    if (subset==FALSE) {
        p3 <- p3 +
            scale_x_discrete(labels= c("Controls",Disease)) +
            scale_fill_manual(values=c("#E7298A","orange","black"))
...
PRS_histogram <- function(PRS,Disease,subset=FALSE,age=0){
    ...
    p2
    if (subset == FALSE) {
        p3 <- p2 + ylab("General Population (%)") + 
            scale_fill_manual(name = "Status",
                              values=c("#E7298A", "orange"),
                              labels= c("Controls",Disease))
    } else {
        p3 <- p2 + ylab(ifelse(subset==Disease, "General Population (%)",paste0(subset," Subset (%)"))) +
            scale_fill_manual(name = "Status",
...
#make plots
p1 <- Violin_plots("T2D","T2D",FALSE,test ="t.test",age=0) %>% label_plot(., "A",25)
p2 <- Violin_plots("MDD","MDD",FALSE,test ="t.test",age=0) %>% label_plot(., "B",25)
p3 <- Violin_plots("MH","MH",FALSE,test ="t.test",age=0) %>% label_plot(., "C",25)
p4 <- Violin_plots("EPI","EPI",FALSE,test = "t.test",age=0) %>% label_plot(., "D",25)
p5 <- PRS_histogram("T2D","T2D",FALSE, age=0) %>% label_plot(., "E",25)
p6 <- PRS_histogram("MDD","MDD",FALSE,age=0) %>% label_plot(., "F",25)
p7 <- PRS_histogram("MH","MH",FALSE,age=0) %>% label_plot(., "G",25)
p8 <-PRS_histogram("EPI","EPI",FALSE, age=0) %>% label_plot(., "H",25)
p9 <- decil_plot("T2D","T2D",FALSE,age=0) %>% label_plot(., "I",25)
p10 <- decil_plot("MDD","MDD",FALSE,age=0) %>% label_plot(., "J",25)
p11 <- decil_plot("MH","MH",FALSE,age=0) %>% label_plot(., "K",25)
p12 <- decil_plot("EPI","EPI",FALSE,age=0) %>% label_plot(., "L",25)
...
ggsave(file="All_figures/FigureSup3.pdf",grid_vertical,width = 22.5,height = 30)
